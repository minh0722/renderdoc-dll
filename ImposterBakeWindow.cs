...
public class ImposterBakeWindow : EditorWindow
{
	...
	private static Void Renderdoc_Initialize;
	private static Void Renderdoc_BeginCapture;
	private static Void Renderdoc_EndCapture;
	...
	private static IntPtr _renderdocLib = IntPtr.Zero;
	
	private static void LoadAllDelegates()
	{
		_renderdocLib = LoadLibrary("renderdoc-csharplib.dll");
		if (_renderdocLib == IntPtr.Zero)
		{
		...
		bool res = LoadDelegate(ref Renderdoc_Initialize, "Initialize");
		res &= LoadDelegate(ref Renderdoc_BeginCapture, "BeginCapture");
		res &= LoadDelegate(ref Renderdoc_EndCapture, "EndCapture");
	}

	private static bool LoadDelegate<T>(ref T functionDelegate, string functionName) where T : class
	{
		if (_renderdocLib.Equals(null))
			return false;

		IntPtr functionAddress = GetProcAddress(_renderdocLib, functionName);
		if(functionAddress == IntPtr.Zero)
		{
			return false;
		}

		functionDelegate = Marshal.GetDelegateForFunctionPointer(functionAddress, typeof(T)) as T;
		return functionDelegate != null;
	}

	private static void RenderdocInitialize()
	{
		Renderdoc_Initialize?.Invoke();
	}

	private static void RenderdocBeginCapture()
	{
		Renderdoc_BeginCapture?.Invoke();
	}

	private static void RenderdocEndCapture()
	{
		Renderdoc_EndCapture?.Invoke();
	}
